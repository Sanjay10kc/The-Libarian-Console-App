...
int flag = 0;
 ...
 scanf("%s",bn);
 while(fread(b->name,sizeof(struct book),1,bk)==1){
...
void transaction(char name[])
{
    ...
    else
    {
        ...
        while(bk!=EOF)
        {
            fread(b->name,sizeof(struct book),1,bk);
            fread(b->author,sizeof(struct book),1,bk);
            //fread(date_sub,sizeof(date_sub),1,bk);
            system("cls");
            printf("%s\t%s\n",b->name,b->author);
            //free(b);
            //free(date_sub);
